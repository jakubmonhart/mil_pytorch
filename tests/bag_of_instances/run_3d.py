...
def create_model(input_len, n_neurons1, n_neurons2, device = 'cpu'):
...
instances, ids, labels = create_bags_simple.create_bags(source_data, source_labels, pos = pos, neg = neg, max_instances = max_instances)
...
dataset = mil.MilDataset_3d(instances, ids, labels, normalize = True, device = device)
...
model = create_model(len(dataset.data[0][0]) , n_neurons1, n_neurons2, device = device)
